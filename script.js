...
function construirDocumento(){
    //coleta todas as revisôes adicionadas
...
gerarPdfBtn.addEventListener("click", () =>{
    ...
    y+=7;
    pdf.setFontSize(12);
    pdf.text(`- Nome: ${doc.s.nome}`, 25, y);
    y += 7;
    pdf.text(`-Cargo: ${doc.responsavel.cargo}`, 25, y);
    y += 7;
    pdf.text(`- Departamento: ${doc.s.departamento}`, 25, y);
    y += 15;

    // Sessão de Palavras-chave
    pdf.setFontSize(14);
    ...
    if(doc.revisoes.length > 0){
        doc.revisoes.forEach((rev, index) => {
            ...
            // Lógico de quebra de lina para o comentário
...
copyBtn.addEventListener("click", () => {
    ...
    navigator.clipboard.writeText(textoParaCopiar).then(() =>{
        ...
        console.erro0("Falha ao copiar o texto: ", err);
